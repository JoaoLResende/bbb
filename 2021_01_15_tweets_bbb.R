...
token <- read_sheet(as_id("1I82vtFzaNlo8NxSQHe4B7OgARG9zu906UDQeGVEId3o"))
...
comparacao_duas_datas <- combinado_ultimos_dois %>%
  ...
  labs(title = "Sentimento em relação a cada Brother no Twitter",
       subtitle = "Diferença entre a última análise (17/01) e a atual(24/01)",
       x = "Bother",
       y = "Pontuação",
       color = "",
       caption = paste0("João Resede, ", Sys.Date()))
...
mensagem <- paste0("Atualização dos dados da análise de sentimentos do BBB dia", sep= ", ", Sys.Date(), sep = ".", "#bbb, #rstats, #bbb22, #dataviz. Essa é uma atualização automática. Cortesia do pacote rtweet.")
